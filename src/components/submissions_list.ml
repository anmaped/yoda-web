  ...
  Lwt.async (fun () ->
      ...
      List.iter
        (fun (sub : Api.Openapi.submission) ->
          ...
          in
          (*let problem =
            match Js.Opt.get sub.problem_id (fun () -> Js.string "?") with
            | s -> Js.to_string s
          in
          let lang =
            match
              Js.Opt.get sub.language (fun () -> Js.string "Unknown")
            with
            | s -> Js.to_string s
          in*)
          let problem = "" in
          let lang = "Unknown" in
          let row = submission_row id problem lang status time_str in
          Dom.appendChild
