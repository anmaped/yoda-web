...
let render ~contest_id ~problem_id () =
  let lang_input = input ~a:[a_placeholder (I18n.t "subform_label_language")] () in
  let code_input = textarea ~a:[a_placeholder (I18n.t "subform_placeholder_source")] (txt "") in
  let submit_btn = button [txt (I18n.t "subform_submit")] in
  let div_form = div [
    div [txt (I18n.t "subform_label_language"); lang_input];
    div [txt (I18n.t "subform_label_source"); code_input];
    submit_btn
  ] in
